...
export const WalletSection: React.FC<WalletSectionProps> = ({ address, ydBalance, isConnected, onRefresh }) => {
  return (
    <section className="mb-6 rounded-3xl bg-gradient-to-br from-white via-sky-50 to-blue-50/80 p-5 shadow-xl shadow-sky-100/80 ring-1 ring-sky-100/80">
      <div className="flex items-center justify-between gap-4">
        ...
        {isConnected && onRefresh && (
          <button
            onClick={onRefresh}
            className="rounded-xl bg-gradient-to-r from-sky-400 to-cyan-400 px-4 py-2 text-sm font-medium text-white shadow-sm shadow-cyan-200/80 transition hover:shadow-md active:scale-[0.98]"
          >
            刷新
          </button>
        )}
