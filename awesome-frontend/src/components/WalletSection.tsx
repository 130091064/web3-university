    ...
    <section className="flex h-full min-h-[260px] flex-col rounded-2xl bg-slate-50/80 p-4 shadow-sm ring-1 ring-slate-100 sm:p-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-1">
          <h2 className="text-base font-semibold text-slate-900">钱包 & 资产概览</h2>
          {isConnected ? (
            <p className="text-xs text-slate-500">
              钱包地址：
              <span className="font-mono text-slate-700">{shorten(address)}</span>
            </p>
          ) : (
            <p className="text-xs text-slate-500">连接钱包后即可查看你的链上资产。</p>
          )}
        </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="rounded-xl bg-white/90 p-3 shadow-sm ring-1 ring-slate-100">
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium text-slate-500">YD 余额</span>
